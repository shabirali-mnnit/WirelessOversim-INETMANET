...
void Chord::changeState(int toState)
{
    ...
    case JOIN:
       ...
       //std::cout << "bootstrapNode" << bootstrapNode << std::endl;

       // bootstrapNode = TransportAddress->setIp(192.168.0.1,1024,UNKNOWN_NAT);
        ...
        if (bootstrapNode.isUnspecified()) {
            ...
            globalNodeList->setBootstrapPeer(thisNode,overlayId);
            changeState(READY);
            updateTooltip();
        }
        break;
...
NodeVector* Chord::closestPreceedingNode(const OverlayKey& key)
{
    ...
    for (int j = successorList->getSize() - 1; j >= 0; j--) {
        ...
        if (successorList->getSuccessor(j).getKey().isBetween(thisNode.getKey(), key)) {
